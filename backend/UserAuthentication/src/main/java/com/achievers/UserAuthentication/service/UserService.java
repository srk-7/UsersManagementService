...
public class UserService implements UserDetailsService {
    ...
    public ResponseEntity<String> registerAdmin(RegisterRequestDto registerRequestDTO) {
        ...
        if (usersRepository.findByUsername(registerRequestDTO.getEmailId()).isPresent()) {
    ...
    // shivaramakrishna
    public ResponseEntity<Admins> getAdminByEmail(String email) {
        Admins admin = (Admins) adminRepository.findByEmailId(email).orElse(null);
        if (admin == null) {
            return ResponseEntity.status(HttpStatus.NOT_FOUND).body(null);
        }
        return ResponseEntity.ok(admin);
    ...
    // shivaramakrishna
    public ResponseEntity<Customers> getCustomerByEmail(String email) {
        Customers customer = (Customers) customerRepository.findByEmailId(email).orElse(null);
        if (customer == null) {
            return ResponseEntity.status(HttpStatus.NOT_FOUND).body(null);
        }
        return ResponseEntity.ok(customer);
